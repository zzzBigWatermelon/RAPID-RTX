'''这个是Develop, Customize, and Publish in Omniverse With Extensions课程中
制作的下拉选择框模块'''
...
class ComboBoxModel(ui.AbstractItemModel):
    ...
    def set_items(self, items: list, default_index: int = 0):
        """
        动态替换下拉列表的所有选项。
        Args:
            items: 字符串列表，如 ["400.5 nm", "412.3 nm", ...]
            default_index: 默认选中项的索引，默认为 0
        """
        # 创建新的 ListItem 列表
        self._children = [ListItem(text) for text in items]
        # 更新默认选中索引
        self._default.as_int = default_index if 0 <= default_index < len(items) else 0
        # 通知 UI 刷新
    ...
    def get_item_value_model_count(self, item):
        """The number of columns获得有几个子类数量"""
        return 1
