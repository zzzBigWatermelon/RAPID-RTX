'''
早前的注释：底下很多的函数具体参数都没有指定,但是也没有报错,
如show_window的value,_visiblity_changed_fn的visible,应该都是继承父类'''
...
class ObservationExtension(omni.ext.IExt):
    ...
    3、目前只需要更改WINDOW_NAME、MENU_PATH和引入的.window、窗口的停靠位置deferred_dock_in,还有aaaExtension和aaaWindow
    '''
    ...
    def on_startup(self, ext_id):
        ...
        print("[rapid.Observation] rapid Observation startup")
        # 1. 注册 Workspace 窗口函数
        ...
        self._menu_items = [
            ...
            MenuItemDescription(name=self.WINDOW_NAME, ticked_fn=self._is_window_visible, onclick_fn=self._toggle_window)
        ]
    ...
    def show_window(self, menu, value):
        ...
        if value:
            if not self._window:
                self._window = ObservationWindow(self.WINDOW_NAME, width=300, height=500)
    ...
    def _on_start_request_received(self, event: carb.eventdispatcher.Event):
        ...
        print("[Rapid.Observation] 收到rapid.simulation的开始请求,正在读取当前UI数据...")
        # 数据请求方的识别sender_id
        ...
        if self._window:
            try:
                current_data = self._window.get_data()
                self._cached_data = current_data  # 更新缓存
            except Exception as e:
                carb.log_warn(f"Window exist but failed to get data (maybe hidden?): {e}, using cache.")
                current_data = self._cached_data
        else:
            print("[Rapid.Observation] 窗口已关闭，使用最后一次缓存的数据")
            current_data = self._cached_data

        # 3. 发送数据回对应的sender
        current_data = {"sender": sender, 'Observation': current_data}
        carb.eventdispatcher.get_eventdispatcher().dispatch_event(
            event_name=SIMULATION_PARAMS_READY,
            payload=current_data
        )
        print(f"[Rapid.Observation] 数据已发送到rapid.Simulation sensor_stage_path: {current_data}")