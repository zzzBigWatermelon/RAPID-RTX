...
class ObservationUtils:
    ...
    def get_ui_value(UI_model):
        ...
        if isinstance(UI_model, list):
            return [ObservationUtils.get_ui_value(m) for m in UI_model]  # 递归提取列表里的每个模型
    ...
    def save_UI_data_to_json(UI_model):
        ...
        # 覆盖旧数据
        existing_data["Observation"] = data
    ...
    def read_json_data_to_UI(UI_model_dict):
        ...
        if not simulation_parameters_file.exists():
            ...
            if default_parameters_file.exists():
                ...
                carb.log_info(f"[rapid.Observation] The extension's default parameters are loaded.: {simulation_parameters_file}")
            else:
                carb.log_warn("[rapid.Observation] No configuration files (simulation_parameters.json) found.")
                return
        ...
        try:
            with open(simulation_parameters_file, 'r', encoding='utf-8') as f:
                ...
                obs_data = json_data.get("Observation", {})
        except Exception as e:
            print(f"[rapid.Observation] Failed to parse JSON file: {e}")
            return
        ...
        for key, value in obs_data.items():
            ...
            # --- B. 处理特殊字段 ( omnidirectional_sampling_view_zenith：从 List 转回 CSV 字符串 ) ---
            elif key == "omnidirectional_sampling_view_zenith":
                if isinstance(value, list):
                    ui_model.as_string = ", ".join(map(str, value))
                else:
        ...
        carb.log_info("[exts rapid.Observation] The JSON parameters have been successfully synchronized to the UI.")