...
(这是针对omni.replicator.core.distribution.sequenc在omni.replicator.core.modify.time连用时, 产生错误,已经作废)
3、181行,self._custom_name_prefix = ""  # 用于存储输出的文件名
4、加入了hdr的输出,数据类型是float16
'''
import io
import json
...
class WorkWriter(Writer):
    ...
    def __init__(
        ...
        # 自定义项
        self._custom_name_prefix = None  # 用于存储输出的文件名
        ...
        # HDR
        if hdr:
            self.annotators.append(AnnotatorRegistry.get_annotator("HdrColor"))
    ...
    def write(self, data: dict):
        ...
        for annotator in data.keys():
            ...
            if annotator.startswith("HdrColor"):
                if multi_render_prod:
                    render_product_path += "HdrColor/"
                self._write_hdr(data, render_product_path, annotator)
    ...
    def set_custom_name(self, name: str):
        """新增：供外部调用的接口，设置当前步的文件名"""
        self._custom_name_prefix = name

    def _write_rgb(self, data: dict, render_product_path: str, annotator: str):
        # 如果有自定义前缀则使用自定义，否则退回默认命名
        if self._custom_name_prefix:
            # render_product_path是文件夹路径
            file_path = f"{render_product_path}{self._custom_name_prefix}.{self._image_output_format}"
        else:
            # 保持原来的默认命名逻辑作为备份
    ...
    def _write_hdr(self, data: dict, render_product_path: str, annotator: str):
        '''
        改进后的 HDR 数据输出：保存为原始二进制 .npy 文件
        这样可以完美保留 float16 的数值，不发生截断
        '''
        hdr_data = data[annotator]  # 这里拿到的是 float16 的 numpy 数组
        # 确定文件名
        if self._custom_name_prefix:
            file_path = f"{render_product_path}{'HDR_'+str(self._custom_name_prefix)}.npy"
        else:
            file_path = f"{render_product_path}HDR_{self._sequence_id}{self._frame_id:0{self._frame_padding}}.npy"

        # 使用 BytesIO 和 np.save 导出二进制流
        buf = io.BytesIO()
        np.save(buf, hdr_data)
        # 通过 Replicator 后端写入 blob
